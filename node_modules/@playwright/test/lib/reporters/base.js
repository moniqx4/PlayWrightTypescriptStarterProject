...
class BaseReporter {
  ...
  onError(error) {
    console.log(formatError(error, _safe.default.enabled).message);
  }
  ...
  _printSlowTests() {
    this.getSlowTests().forEach(([file, duration]) => {
      console.log(_safe.default.yellow('  Slow test: ') + file + _safe.default.yellow(` (${(0, _ms.default)(duration)})`));
    });
  }
...
function formatFailure(config, test, options = {}) {
  ...
  for (const result of test.results) {
    ...
    const {
      tokens: resultTokens,
      position
    } = formatResultFailure(test, result, '    ', _safe.default.enabled);
